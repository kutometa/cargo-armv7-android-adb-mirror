...
OWNER="kutometa"
IMAGENAME="cargo-armv7-android-adb"
BUILD_FLAGS="--squash"
#BUILD_FLAGS=""
...
# Test new image
RESULT=$(docker run --rm --net=host -v "$(realpath .)/test-crate":/crate "$OWNER/$IMAGENAME:$TAG" run -q) || {
    echo "$RESULT" 1>&2
    exit 1
}
...
echo "#!/bin/bash"'
docker run --rm --net=host -v "$(realpath .)":/crate "'"$OWNER/$IMAGENAME:$TAG"'" "$@"
' > "built/$IMAGENAME-$TAG"
